...
amyloid <- read_csv("mci_amyloid.csv", skip=1) %>%
  janitor::clean_names() %>%
  pivot_longer(cols = starts_with("time_"), names_to = "time", values_to = "measurement")
amyloid
```
...
There are `r nrow(distinct(amyloid$study_id))` observations in the dataset. The average baseline time is `r round(mean(distinct(amyloid$baseline, na.rm=TRUE)),digits = 2)` years.
